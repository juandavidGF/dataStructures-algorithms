...
bool isFullBinaryTree(struct Node *root, int a) {
	cout << a;
  ...
  if ((root->left) && (root->right))
    return (isFullBinaryTree(root->left, root->item) && isFullBinaryTree(root->right, root->item));
...
int main() {
  ...
  if (isFullBinaryTree(root, root->item))
